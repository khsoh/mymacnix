...
ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=$KNOWN_HOSTS $REMOTE_HOST "cat > ~/.deploy/token" < <(op service-account create "$SA_NAME" "${VAULTS[@]}" --expires-in=10m --raw)
