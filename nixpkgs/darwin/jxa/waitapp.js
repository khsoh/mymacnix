...
function run(argv) {
  ...
  const installedApp = app.doShellScript(
    `mdfind "kMDItemFSName == '${XAPP}' && kMDItemKind == 'Application'"`,
  );
  if (!installedApp) {
    return;
  }
  ...
  try {
    ...
    do {
      ...
      if (xproc.length > 0) {
        // Now wait for menuBars to appear
        do {
          if (
            xproc[0].menuBars.length > 0 &&
            xproc[0].menuBars[0].menuBarItems.length > 0
          ) {
            return;
          }
          delay(1);
