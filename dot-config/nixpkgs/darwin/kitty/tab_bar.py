...
def get_cpu():
    px = os.popen("/usr/bin/top -l 2|grep -E '^CPU'|tail -1")
    out = px.read()
    print(f"Current CPU Usage: {out}")
    px.close()
    # out = subprocess.getoutput("/usr/bin/top -l 2|grep -E '^CPU'|tail -1|awk '{ print $3 + $5\"%\" }'")
    # return None
    if len(out) > 0:
        return { "icon": "  ", "color": "#2a9d8f", "text": out }
    else:
        return None
